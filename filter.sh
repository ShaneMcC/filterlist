...
IS_SET=$(whois -h whois.radb.net $1 | grep -i ^as-set: | awk -F: '{print $1}' | sort -u)
...
IP_LIST=$(printf "%s\n" $IP_LIST_UNSORTED | sed -r 's/:[0]+([0-9a-f]+)/:\1/g' | sed -r 's/[:][:0]+[:][:0]+/::/' | sort -t . -k 1,1n -k 2,2n -k 3,3n -k 4,4n | uniq -i)
